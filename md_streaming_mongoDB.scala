    ...
    { pricesRDD =>
      if (!pricesRDD.isEmpty)  // data exists in RDD
      {
        ...
        // MongoSpark.save(document, writeConfig)
         ...
         println("Total Prices added to the collection so far: " +totalPrices+ " , Runnig for  " + (endTimeQuery - startTimeQuery)/(1000*60)+" Minutes")
         ...
         val lookups = df.filter('priceInfo.getItem("ticker") === tickerWatch && 'priceInfo.getItem("price") > priceWatch)
         if(lookups.count > 0) {
